...
import { Commission } from "interfaces/profile";
...
} from "commission/request";

import Bank from "commission/containers/bank";
...
class ProfilePost extends Component<Props, State> {
    ...
    init = async () => {
        try {
            const { type, commission } = this.props;

            const commissionId = _.get(commission, "id");
            const entity = _.toLower(_.get(commission, "entity"));
            const country = _.toUpper(_.get(commission, "country"));
    ...
    sendRequest = async () => {
        const { waitingForUpload, files, items, attrs, attrsValue } =
            this.state;
        const { type, commission, changeNav } = this.props;
