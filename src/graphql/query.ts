...
export const SHOP_PRODUCTS = gql`
    query getProducts($merchant: ID!) {
        topSales: products(
            where: { merchant: $merchant, status: "online", salePrice_eq: 0 }
            sort: "sales:desc"
            start: 0
            limit: 10
        ) {
        ...
        products(
            where: { merchant: $merchant, status: "online" }
            sort: "createdAt:desc"
        ) {
            id
            title
...
export const SHOP_EXPLORE = gql`
    query getProducts($merchant: ID!) {
        products(where: { merchant: $merchant }, sort: "createdAt:desc") {
            category {
                id
                title
            }
...
export const COMMISSION = `
    query getCommission($id: String!) {
        providers(limit: 1, where: { user: $id }) {
            id
            first_name
            last_name
            gender
